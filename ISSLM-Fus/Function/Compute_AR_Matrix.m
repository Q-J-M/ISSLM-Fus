function  A     =  Compute_AR_Matrix( im, AR_D,centroids,par)
...
[h w]      =  size( ext_im );
LP_filter  =  fspecial('gaussian', 7,  1.5);
...
for  row  = 1:h
    for  col  =  1:w
        ...
        if  flag(row,col)==0
            ...
            v          =  X(:,off_cen);
            dis        =  sum( (v(:,ones(1,cls_num) ) - Codeword).^2 );           
            [m_dis Indics]  =  sort( dis );
            
            w_arr      =  1./( dis( Indics(1:num_mod) )+eps );
